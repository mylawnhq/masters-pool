...
export function deriveTournamentState(golferStats, liveMode) {
  ...
  // ROUND-FLIP HEURISTIC: if current_round is still 2 in the DB but the
  // thru values look like tee times (contain AM/PM or ":") rather than
  // hole numbers, the round has flipped to R3 and the cron just hasn't
  // updated current_round yet. In this case the cut is definitely done.
  // This catches the gap between R3 starting and the next cron tick.
  const looksLikeNextRound =
    currentRound === 2 &&
    activeGolfers.length > 0 &&
    activeGolfers.some(v => {
      const t = String(v.thru || '');
      return /AM|PM|:/i.test(t);
    }) &&
    !activeGolfers.some(v => {
      // If any active golfer has a numeric thru between 1-17, R2 is still
      // genuinely in progress (not a tee time, not finished).
      const t = String(v.thru || '').trim();
      const n = parseInt(t, 10);
      return Number.isFinite(n) && n >= 1 && n <= 17;
    });

  // Also check: if ANY golfer has status "cut", the cut has been applied
  // by ESPN and is definitively finalized.
  const anyCutGolfers = vals.some(v => v.status === 'cut');
